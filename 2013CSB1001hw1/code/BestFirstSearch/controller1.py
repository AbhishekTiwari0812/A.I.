...
def getHvalue(current):
	...
	returning the euclidian distance'''
	global diamond_y
	global diamond_y
	return math.sqrt((diamond_x-current.x)**2+(diamond_y-current.y)**2);


def BestFirstSearch(map,x,y):
	...
	for i in range(0,len(board)):
		for j in range(0,len(board[i])):
			if(board[i][j]==1):
				...
				tempExploredNodes,tempSearchQueue,tempShortestPath = BestFirstSearch(map,i,j)
