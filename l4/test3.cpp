...
struct literal{
	int block1;	
	int block2;			
	int id;		//==1 block1 is on block2, ==2  block1 is on table, ==3 block is held by the arm, ==4 block is clear.==5 is id of the arm
};
...
*/
// 1=on(a,b) 2= onTable(a)
vector<literal> preconditions(Action &a){
	vector<literal> v;
	literal temp;
	switch(a.id){
		case 1:		//pick a.
			//on table
			temp.id=2;
			temp.block1=a.block1;
			temp.block2=a.block1;
			v.push_back(temp);
			//is arm free
			temp.id=5;
			v.push_back(temp);
		break;
		case 2:	//unstack(a,b)
			//check whether a is on b.
			temp.id=1;
			temp.block1=a.block1;
			temp.block2=a.block2;
			v.push_back(temp);
			//check whether a is clear
			temp.id=6;
...
vector<Action> get_action(literal &a){
	vector<Action> v;
	switch(a.id){
		case 1:		//on(a,b)
			Action temp1(4,a.block1,a.block2);
			v.push_back(temp1);
		break;
		case 2:		// onTable(a)
			Action temp2(3,a.block1,block1);
			v.push_back(temp2);
		break;
		case 3:			//held by the arm from the table
			Action temp3(1,a.block1,block1);
			v.push_back(temp3);
		break;
		case 4:		//held by the arm from another block
			Action temp4(2,a.block1,a.block2);
			v.push_back(temp4);
		break;
		case 5:			//arm free
			Action temp5(4,a.block1,a.block1);
			v.push_back(temp5);
		break;
		case 6:		//block is clear after unstacking

		break;
		case 7:		//block is clear after releasing
